...
Vite supports importing `.ts` files out of the box. Learn what Vite does with TypeScript files and how to configure TypeScript and Vite in the [TypeScript integration section](/integration-typescript).
...
All modern frameworks maintain integrations with Vite. Most framework plugins are maintained by each framework team, with the exception of the official Vue and React Vite plugins that are maintained in the Vite organization:
...
Check out the [Framework Integration guide](/integration-frameworks) for more information.
